...
import { Partner, PartnerResponse, PartnersResponse } from "@/types/partners";
...
export const useGetPartner = ({ id, query }: { id: string; query: {} }) => {
  const endpoint = new ApiClient<any, PartnerResponse>(`/partners/${id}`);
  return useQuery({
    queryFn: endpoint.get,
    queryKey: ["partner", id],
    meta: { params: { ...query } },
